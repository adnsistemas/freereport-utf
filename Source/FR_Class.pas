...
procedure TfrMemoView.SaveToFR3Stream(Stream: TStream);
var
  fs: Integer;
  f1, f2: Integer;
  s: String;
...
//    WriteStr(' Child="' + ChildBand + '"');
  ...
  if (Flags and flBandNewPageAfter) <> 0 then
    WriteStr(' StartNewPage="True"');
  if (Flags and flBandPrintifSubsetEmpty) <> 0 then
    WriteStr(' PrintIfDetailEmpty="True"');
  if (Flags and flBandPageBreak) <> 0 then
    WriteStr(' AllowSplit="True"');
  if (Flags and flBandOnFirstPage) = 0 then
    WriteStr(' PrintOnFirstPage="False"');
  if (Flags and flBandOnLastPage) = 0 then
    WriteStr(' PrintOnLastPage="False"');
...
//    WriteStr(' PrintChildIfInvisible="True"');
  ...
  if Script.Count > 0 then
    WriteStr(' OnBeforePrint="' + Name + 'OnBeforePrint"');
  if BandType in [btMasterData, btDetailData, btSubDetailData] then
  begin
    ds := frFindComponent(CurReport.Owner, DataSet) as TfrDataSet;
    if ds <> nil then
      WriteStr(' DataSetName="' + StrToXML(ds.Name) + '"');
  end;
  if BandType = btGroupHeader then
    WriteStr(' Condition="' + StrToXML(GroupCondition) + '"');
end;
...
procedure TfrSubReportView.SaveToFR3Stream(Stream: TStream);
...
procedure TfrLineView.SaveToFR3Stream(Stream: TStream);
...
var
  ...
  t: TfrView;
begin
  if CurReport.FR3Stream then
  begin
    SaveToFR3Stream(CurReport, Stream);
    Exit;
  end;
...
var
  Stream: TFileStream;
begin
  Stream := TFileStream.Create(ChangeFileExt(FName, '.fr3'), fmCreate);
  try
    FR3Stream := True;
    SaveToStream(Stream);
  finally
